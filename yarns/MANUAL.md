...
### Play mode
- Setting `PM (PLAY MODE)` sets 
- `MANUAL`: Output is generated by the MIDI controller only
- `SEQUENCER`: Output is generated by the sequencer (and by the MIDI controller depending on `SI (SEQ INPUT RESPONSE)` -- see below)
- `ARPEGGIATOR`: Output is generated by the arpeggiator, which may be driven by the sequencer (see below)
