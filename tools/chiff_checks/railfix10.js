// Refine end-of-attack darkening: DART DENSITY ramps 1/2 -> 1 as the room
// above shrinks (engage = 1 - upRoom/amp). More motion + denser down-spikes
// near the rail; aim balance (and thus the dip) unchanged.
const fs=require('fs');
const html=fs.readFileSync(process.argv[2],'utf8');
let src=html.slice(html.indexOf('const FS = 45000'),html.indexOf('function dialed('));
// current sim = balanced-aim sag=1/8; parametrize by replacing the dart block
const CUR=`      if ((r & 0xFFFF) < 0x8000) {                      // ~half the samples dart
        const upRoom = maxLevel - dialed;
        const sag = Math.min(SAG_FRACTION * amp, Math.max(0, amp - upRoom));
        const u = Math.min(amp, upRoom + sag);          // up-aim kisses the rail from the sagged center
        const pUp = (amp - sag) / (u + amp);            // frequent short ups, rare full downs
        dartTgt = (((r >>> 17) & 0x7FFF) / 32768 < pUp) ? u : -amp;
      }`;
const mk=(sagf,dens)=>`      {
        const upRoom = maxLevel - dialed;
        const sag = Math.min(${sagf} * amp, Math.max(0, amp - upRoom));
        const u = Math.min(amp, upRoom + sag);
        const pUp = (amp - sag) / (u + amp);
        const engage = ${dens} ? Math.max(0, Math.min(1, 1 - upRoom / amp)) : 0;
        const dartThresh = 0x8000 * (1 + engage);
        if ((r & 0xFFFF) < dartThresh) {
          dartTgt = (((r >>> 17) & 0x7FFF) / 32768 < pUp) ? u : -amp;
        }
      }`;
...
for(const [sagf,dens,name] of [['1/8',false,'sag1/8 (current) '],['1/16',true,'sag1/16 + density'],['1/12',true,'sag1/12 + density']]){
  const render=build(mk(sagf,dens));
