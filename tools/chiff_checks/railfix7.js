// Same harness as railfix6 but balanced-aim scoped to the TOP rail only:
// u = min(amp, upRoom); P(up among darts) = amp/(u+amp); down always full -amp.
// Away from the top u=amp -> 50/50 (bit-identical statistics to current).
const fs=require('fs');
...
const BAL=`const u = Math.min(amp, maxLevel - dialed);
        const pUp = amp / (u + amp);
...
const render=build(BAL);
analyze(render,shortP,shortW,'BAL-TOP short ');
analyze(render,longP,longW,'BAL-TOP 8s    ');
analyze(render,Object.assign({},shortP,{amount:127}),shortW,'BAL-TOP amt127');
