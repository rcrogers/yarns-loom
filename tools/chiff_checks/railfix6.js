// BALANCED-AIM: up target u=min(amp,upRoom), down target d=min(amp,dnRoom),
// P(up among darts) = d/(u+d) -> zero-mean by construction (no notch), targets
// always reachable (no pins), rare-but-full darts keep broadband at rails.
// Away from rails u=d=amp -> plain 50/50 (unchanged).
const fs=require('fs');
...
const BAL=`const u = Math.min(amp, maxLevel - dialed), dn = Math.min(amp, dialed);
        const pUp = dn / (u + dn);
        dartTgt = (((r >>> 17) & 0x7FFF) / 32768 < pUp) ? u : -dn;`;
...
function analyze(render,p,wins,label){
  ...
  let hopMin=1e18,hopSum=0,hopN=0;                       // darkest 64-sample hop, late attack
  ...
  console.log(`${label}: rail ${atRail} (longest ${(longest/45).toFixed(2)}ms) darkestHop ${(hopMin/(hopSum/hopN)*100).toFixed(0)}%`);
...
for(const [patch,name] of [[ORIG,'v1(RAIL_CAP=8)'],[BAL,'BALANCED-AIM ']]){
  const render=name.startsWith('v1')?(()=>{const mod={};eval(html.slice(html.indexOf('const FS = 45000'),html.indexOf('function dialed('))+'\nmod.render=render;');return mod.render;})():build(patch);
  analyze(render,shortP,shortW,name+' short');
  analyze(render,longP,longW,name+' 8s   ');
  analyze(render,Object.assign({},shortP,{amount:127}),shortW,name+' amt127');
}
