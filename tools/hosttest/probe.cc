...
int main(int argc, char** argv) {
  ...
  printf("blk stage  targetSmp  slewTime  stageSlewTime"
  ...
  for (int b = 0; b < blocks; ++b) {
    ...
    bool floor_binds = env.slew_time_log2_q5_27_ > env.stage_slew_time_log2_q5_27_;
    if (b >= from_block) {
      const double rate = pow(2.0, -SlewTimeLog2Of(env.slew_time_log2_q5_27_));
      ...
      printf("%3d %5d %10u %8.3f %13.3f %8.5f %9.0f %8.1f %8.2f %10.6f %s\n",
             b, (int)env.stage_, env.chiff_target_samples_,
             SlewTimeLog2Of(env.slew_time_log2_q5_27_),
             SlewTimeLog2Of(env.stage_slew_time_log2_q5_27_),
