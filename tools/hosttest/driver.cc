...
static void RenderMs(double ms) {
  ...
  for (size_t i = 0; i < n; i += kAudioBlockSize) {
    ...
    if (g_chiff_trace) {
      ...
      printf("%u %u %d\n", env.chiff_amount_q30_,
             env.chiff_slew_time_log2_q5_27_, env.ChiffSlewInput_q30());
      continue;
    }
    if (g_slew_trace) {
      // Third field is the slowest slew this note's duration allows. The
      // engine's own value, never re-derived here: a second derivation would be
