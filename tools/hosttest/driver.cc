...
// look fine either way -- so this reads value_without_bias_q30_ between blocks, which is
  ...
  for (size_t i = 0; i < n; i += kAudioBlockSize) {
    ...
    if (g_slew_trace) {
      ...
      printf("%u %d %u\n", env.chiff_slew_time_log2_q5_27_,
             env.chiff_slew_input_fraction_q30_, env.chiff_slew_time_at_amount_zero_q5_27_);
      continue;
    }
    ...
    if (g_value_trace) { printf("%d\n", env.value_without_bias_q30_); continue; }
    if (g_value_range) {
      if (env.value_without_bias_q30_ < g_value_min) g_value_min = env.value_without_bias_q30_;
      if (env.value_without_bias_q30_ > g_value_max) g_value_max = env.value_without_bias_q30_;
      continue;
    }
...
int main(int argc, char** argv) {
  ...
  // The driver names AMOUNT as the panel setting; NoteOn takes the
  // modulate_7_13 form, six bits below the knob step.
  const uint16_t amount_q7_6 = static_cast<uint16_t>(amount) << 6;
  ...
  if (strcmp(scenario, "basic") == 0) {
    ...
    env.NoteOn(adsr, 0, max_target, amount_q7_6, chiff_audible_samples);
    RenderMs(gatems);
    env.NoteOff();
    ...
    env.NoteOn(adsr, 0, 16383, amount_q7_6, chiff_audible_samples);
    RenderMs(60);
    env.NoteOff();
    RenderMs(1000);
    ...
    env.NoteOn(adsr, 16383, 0, amount_q7_6, chiff_audible_samples);
    RenderMs(2000);
    env.NoteOff();
    RenderMs(1000);
